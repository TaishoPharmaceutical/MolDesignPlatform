...
def scaled_dot_product_attention(q, k, v, mask, additional_weights):
...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(self, v, k, q, mask, additional_weights):
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training,
        ...
        x_ = self.ffn(x)
        x_ = self.dropout3(x, training=training)
...
class Encoder(tf.keras.layers.Layer):
    def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size,
               maximum_position_encoding, rate=0.1, use_embedding=True, use_pos_enc=True):
...
class Transformer(tf.keras.Model):
    ...
    def call(self, inp, tar, training, enc_padding_mask,
        ...
        dec_output, attention_weights = self.decoder(
            tar, enc_output, training, look_ahead_mask, dec_padding_mask, pos_enc)

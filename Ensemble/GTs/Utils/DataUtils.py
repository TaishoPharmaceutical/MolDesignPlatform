...
def encode_to_array(cmols, max_len=MAX_LEN):
    ...
    adj_lists = np.zeros([num_mols, 5, max_len, max_len], dtype=np.float32)
...
class FeatMol():
    ...
    """
    def __init__(self,mol,useBO=True):
        cmol = self.conv_mol(mol,useBO)
    ...
    def conv_mol(self, mol, useBO=True):
        ...
        if useBO:
            ...
            adj = np.ceil((adj>=1.5)*0.1+adj)
            adj = np.identity(5)[adj.astype(np.int64)]
            adj = [adj[:,:,i] for i in range(5)]
            adj = [adj_matrix] + adj[1:]
            #adj_matrix = np.concatenate([np.stack(adj)]*4, 0)

        return [np.array(nodes, dtype=np.float32), adj_matrix.astype(np.float32)]
...
def read_csv_auto(file_name, smiles="SMILES", max_len=MAX_LEN, save_file=None):
    df=pd.read_csv(file_name)
    ...
    for i,label in enumerate(res_labels):
        if len(set(label))<=3: #3 values are expected: positive, negative, nan
            cls_tasks.append(tasks[i])
        else:
            reg_tasks.append(tasks[i])
    ...
    return res, reg_tasks, cls_tasks
